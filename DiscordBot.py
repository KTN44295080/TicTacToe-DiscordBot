...
async def on_message(message):
    if not gameStats.isMatch or not message.author.id == gameStats.playerIds[gameStats.turnPlayer]:
        return
    else:
        ...
        if result > 0 and result <= gameStats.squareCount:
            ...
            gameStats.turn += 1
            if LineCheck() != 0:
                await message.channel.send(f"{gameStats.playerNames[gameStats.turnPlayer]}さんの勝利です！")
                gameStats.Reset()
                return
            ...
            if len(gameStats.playerNames) < 2 :
                ...
                await message.channel.send(f"{gameStats.playerNames[gameStats.turnPlayer]}さんのターンです")

async def get_players(ctx) :
    return [user.display_name for user in bot.get_channel(ctx.interaction.channel_id).members if user.display_name.startswith(ctx.value) and not user.bot]


@bot.slash_command(description = "三目並べを開始します")
async def play(ctx, player: Option(str, "対戦相手を指定してください。指定しない場合、コンピューターが相手になります。", autocomplete = get_players) = None, square : Option(int, "何マス×何マスでプレイするか指定してください。指定しない場合、3×3になります。") = 3) :
    if square <= 0 :
    ...
    if(not player == None) :
        gameStats.playerIds.append([user.id for user in bot.users if user.display_name == player][0])
        gameStats.playerNames.append(player)

    await ctx.respond(f"{square}×{square}マスで始めます。\n{ctx.author.display_name}さん、置く場所を1~{gameStats.squareCount}で指定してください")
    textField = ""
    loopCount = 0
...
async def force_end(ctx, square):
    if (not square == 0):
        ...
        await ctx.respond(f"{square}x{square}マスで始めます。おはよう{ctx.author.display_name} お前の勝ちだよ")
    return
...
def ReachCheck():
    for i in reversed(range(1, 3)):
        for y in range(gameStats.division):
            if gameStats.fields[y].count(i) == gameStats.arrangeCount - 1 and 0 in gameStats.fields[y]:
                if i != 0:
        ...
        if diagonal.count(i) == gameStats.arrangeCount - 1 and 0 in diagonal:
            ...
            return index, index
        diagonal = [gameStats.fields[n][gameStats.division - 1 - n] for n in range(gameStats.division)]
        if diagonal.count(i) == gameStats.arrangeCount - 1 and 0 in diagonal:
            index = diagonal.index(0)
